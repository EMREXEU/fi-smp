...
public class ElmoXmlImportHelper {
  ...
  private ElmoResult resultFromDegree(List<ElmoResult> results, Node los, XPath xpath)
      throws Exception {
  ...
  private ElmoResult resultFromModule(List<ElmoResult> results, Node los, XPath xpath)
      throws Exception {
    ElmoResult res = new ElmoResult();
    getLearningOpportunityValues(los, xpath, res);
  ...
  private void getLearningOpportunityValues(Node los, XPath xpath, ElmoResult res)
      throws Exception {
  ...
  private String getLevel(Node los, XPath xpath) throws Exception {
    String simpleLevel = getValueForTag(los, "level");
      if (!simpleLevel.isEmpty()) {
          ...
          return getPreferredContentValue(los, xpath, "qualification/educationLevel", "xml:lang",
              "en");
      }
  }

  private void loopChildren(List<ElmoResult> results, Node los, XPath xpath, String path)
      throws Exception {
  ...
  private String getPreferredContentValue(Node node, XPath xpath, String expr, String itemName,
      String preferredValue) throws XPathExpressionException {
